...
export default async function RoutePlannerPage({ searchParams }: PageProps) {
	...
	const vehicleCapacityById = new Map(
		(vehiclesRaw ?? []).map((v) => [
			v.id,
			{
				kidsSeats: Number(v.kids_seats ?? 0),
				boosterSeats: Number(v.booster_seats ?? 0),
			},
		]),
	);

	for (const r of existingRoutes) {
		const capacity = vehicleCapacityById.get(r.vehicle_id) ?? {
			kidsSeats: 0,
			boosterSeats: 0,
		};
		...
		vehicleRoutes.push({
			...
			vehicleName: r.vehicle_name_snapshot,
			status: r.status,
			totalDistanceKm: r.total_distance_km,
			driverName: r.driver_name_snapshot,
			helperName: r.helper_name_snapshot,
			stops: routeStops,
			kidsSeats: capacity.kidsSeats,
			boosterSeats: capacity.boosterSeats,

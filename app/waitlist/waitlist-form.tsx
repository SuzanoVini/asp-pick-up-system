...
export function WaitlistForm({ entry, onClose }: WaitlistFormProps) {
	...
	const toggleDay = (day: string) => {
		setIntendedDays((prev) => prev.includes(day) ? prev.filter((d) => d !== day) : [...prev, day]);
	};
	...
	return (
		<div className="rounded-lg border border-gray-200 bg-white p-6">
			<h3 className="mb-4 text-lg font-semibold">{isEdit ? "Edit Waitlist Entry" : "Add to Waitlist"}</h3>
			<form onSubmit={handleSubmit}>
				<FormField label="Child Name" required error={errors.child_name}>
					<input type="text" value={childName} onChange={(e) => setChildName(e.target.value)} className={inputClass} />
				</FormField>
					...
					<div className="flex gap-3">
						{DAYS.map((day) => (
							<label key={day} className="inline-flex items-center gap-1.5 cursor-pointer">
								<input type="checkbox" checked={intendedDays.includes(day)} onChange={() => toggleDay(day)} className="h-4 w-4 rounded border-gray-300" />

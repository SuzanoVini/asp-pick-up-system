...
export async function getStaff(supabase: SupabaseClient, filters?: { isActive?: boolean }) {
...
export async function getStaffById(supabase: SupabaseClient, id: string) {
	const { data, error } = await supabase.from("asp_staff").select("*").eq("id", id).single();

	if (error) throw error;
	return data;
}

export async function createStaffMember(supabase: SupabaseClient, input: Record<string, unknown>) {
